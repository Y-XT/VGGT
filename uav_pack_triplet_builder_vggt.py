...
def get_numeric_idx(file_or_name: str) -> Optional[int]:
    """
    从文件名（或路径）中抽取十进制数字，优先取文件名末尾的数字串；
    若无匹配则返回 None。
    例：0000000002.jpg -> 2；frame_00123.png -> 123。
    """
...
def idx_for_log(frames: list, i: int) -> int:
    """
    写 JSON/日志时使用的 idx：优先使用文件名中的数字；否则回退为列表下标 i（0 基）。
    """
...
def frame_ref(frames: list, i: int) -> dict:
    """
    统一生成带 file 与 idx 的引用对象（用于 JSON/日志）。
    """
