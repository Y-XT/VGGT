...
def compute_U_and_disp_median(depth_t: np.ndarray,
                              ...
                              eps_px: float, delta_px: float) -> Tuple[Optional[float], Optional[float]]:
    """方案A（整幅面积比）版本：
      - U：在 in-bounds & z>0 且 eps<=disp<=delta 的像素，按整幅图像进行归一化的比例；
            * 无置信度：U = usable_count / N
            * 有置信度：U = sum(conf * usable) / sum(conf_full)
      - disp_med：可用区域的视差中位数（未加权）。
      - 若无可用像素，返回 (None, None) 以便上层当作无效候选处理。
    """
    D = _as_hw_depth(depth_t)
    H, W = D.shape
    N = float(H * W)

    # 重投影到源帧，再回到中心帧坐标，得到源像素坐标与深度
...
def evaluate_side(frames: list,
    ...
    U, d_med = compute_U_and_disp_median(D_t, K_all[ct], E_all[ct], K_all[cs], E_all[cs], conf_t,
                                         eps_px=eps_eff, delta_px=delta_eff)
    if (U is None):
        _append_jsonl(geom_rejects_path, {
            "seq": str(seq_rel),
            "center": frame_ref(frames, t),
    ...
    for t in tqdm(range(N), desc=str(seq_rel), leave=False):
        ...
        if (best_prev is None) or (best_next is None):
            _append_jsonl(geom_rejects_path, {
                ...
                "detail": {"best_prev": best_prev is not None, "best_next": best_next is not None}
            })
            continue
...
def build_argparser():
    ...
    ap.add_argument("--image_root",   type=str, required=False,
                    default="/mnt/data_nvme3n1p1/dataset/UAVid2020/uavid_v1.5_official_release/China_tri/All",
                    help="原始图像根目录（与阶段一一致）")
    ap.add_argument("--metrics_root", type=str, required=False,
                    default="/mnt/data_nvme3n1p1/dataset/UAVid2020/uavid_v1.5_official_release/China_tri/tri",
    ...
    # 逐帧质量硬阈（过滤极差样本）
    ap.add_argument("--min_lap_p",    type=float, default=0.10,
